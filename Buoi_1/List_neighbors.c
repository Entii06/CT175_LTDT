...
List neighbors(Graph *G, int x){
  ...
  int v; //vertex đỉnh
  for(v=1; v <= G->n; v++){
     if(v!=x){ //nếu đỉnh v khác đỉnh x
       int e; //edge cung
       for(e=1; e <= G->m; e++){
         //nếu đỉnh v thuộc cung e và đỉnh x cũng thuộc cung e thì đỉnh v là hàng xóm (kề) với đỉnh x
         /*
         ví dụ:
         cung 1: 4 3
         thì G->[4][1] = 1
             G->[3][1] = 1
             cả hai đỉnh đều thuộc cung 1 => là hàng xóm
         */
